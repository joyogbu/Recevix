...
Deno.serve(async (req) => {
	...
	try {
		...
		const { data: merchant, error: merchantError } = await supabase
			.from("merchants")
			.select("circle_user_id, wallet_status, merchant_id, merchant_name")
		...
		//check if user has a wallet in circle
		const listCircleResponse = await fetch("https://api.circle.com/v1/w3s/wallets?pageSize=10&order=DESC", {
			method: "GET",
			headers: {
				Authorization: `Bearer ${Deno.env.get("CIRCLE_API_KEY")}`,
		...
		const listCircleData = await listCircleResponse.json();
		console.log(listCircleData);
		if (!listCircleResponse.ok) {
			console.error("Failed to fetch wallets:", listCircleData);
			return Response.json(listCircleData, {
				status: listCircleResponse.status,
				headers: corsHeaders,
			});
		}
		...
		const wallet = listCircleData.data.wallets[0];
		if(wallet) {
		...
		//go ahead an create a wallet
		console.log({userToken, merchant,});
